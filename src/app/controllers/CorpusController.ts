...
export class CorpusController {
  ...
  public async import(
    ...
    return await this.annotatedCorpusImportService.import(
      importer,
      corpusName,
      annotationSetName,
      files
  ...
  public async export(
    ...
    @QueryParam("exporterName") exporterName: string,
    @QueryParam("iob") iob?: boolean
  ): Promise<Return.DownloadBinaryData> {
    if (!exporterName) throw new BadRequestError("Exporter is not defined.");
    return new Return.DownloadBinaryData(
      await this.annotatedCorpusExportService.export(
        exporterName,
        corpusId,
        iob
      ),
      "application/zip",
      `export-corpus-${corpusId}_${dateFormat("yyyymmdd_HHMMss")}.zip`
    );
  }
  ...
  public async list(
  ...
  ): Promise<Corpus[] | number> {
    if (count) {
      if (searchFilter) {
        return this.corpusRepository.count({
          where: {
            [Op.and]: [searchFilter.map((s) => SearchFilter.toSequelize(s))],
          },
        });
      }
      return this.corpusRepository.count();
    ...
    let options = {
      limit,
      offset,
      order: [[sortField, sortOrder] as OrderItem],
    };
    if (searchFilter) {
  ...
  public async create(corpus: Corpus): Promise<Corpus> {
    ...
    if (corpus.annotationSets) {
      for (const annotationSet of corpus.annotationSets) {
        newCorpus.addAnnotationSet(
          this.annotationSetRepository.build(annotationSet)
        );
      }
    }
  ...
  @GET
  public async listAnnotationSets(
    @PathParam("corpusId") corpusId: number
  ...
  @PUT
  public async addAnnotationSet(
    @PathParam("corpusId") corpusId: number,
    @PathParam("annotationSetId") annotationSetId: number
  ): Promise<void> {
    let corpus = await this.corpusRepository.read(corpusId);
    let annotationSet = await this.annotationSetRepository.read(
      annotationSetId
    );
  ...
  @Path(":corpusId/annotationset/:annotationSetId")
  @DELETE
  public async removeAnnotationSet(
    @PathParam("corpusId") corpusId: number,
    @PathParam("annotationSetId") annotationSetId: number
  ): Promise<void> {
    let corpus = await this.corpusRepository.read(corpusId);
