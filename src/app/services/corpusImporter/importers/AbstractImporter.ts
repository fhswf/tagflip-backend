...
export const Importer = <_T extends AbstractImporter>(name: string) => {
  ...
  return <T extends new (...args: any[]) => AbstractImporter>(
    constructor: T
  ) => {
...
export default abstract class AbstractImporter {
   ...
   * @private
   */
  private static importers: Map<string, AbstractImporter> = new Map<
    string,
    AbstractImporter
  >();
  ...
  public static forName(name: string): AbstractImporter {
    if (AbstractImporter.importers.has(name)) {
      const knownImporter = AbstractImporter.importers.get(name);
      if (knownImporter) return knownImporter;
    }
  ...
  async import(
    ...
    try {
      const [zipFiles, regularFiles] = _.partition(files, (f) =>
        [
          "application/zip",
      ...
      for (const file of zipFiles) {
        ...
        await readableBuffer
          .pipe(unzipper.Extract({ path: tmpZipFolder.name }))
          .promise();

        const extractedFiles = await new Promise<string[]>(
          (resolve, reject) => {
      ...
      const importDocuments: ImportDocument[] = await this.doImport(
        corpusName,
        annotationSetName,
        totalFiles
      );
  ...
  private async persist(
    ...
    for (const importDocument of documents) {
      var extension = path.extname(importDocument.fileName);
      var file = path.basename(importDocument.fileName, extension);
          ...
          if (!annotations.get(tag.annotation.name))
            throw Error(
              "Annotation with name " +
                tag.annotation.name +
                " is undefined though its being found in set of existing annotations."
            );
          // @ts-ignore
          annotation = annotations.get(tag.annotation.name);
        } else {
          annotation = await this.annotationRepository.getByName(
            tag.annotation.name
          );
          if (!annotation) {
            if (!fallbackAnnotationSet) {
              const mayBeAnnotationSet = await this.annotationSetRepository.getByName(
                annotationSetName
              );
              fallbackAnnotationSet = mayBeAnnotationSet
                ? mayBeAnnotationSet
