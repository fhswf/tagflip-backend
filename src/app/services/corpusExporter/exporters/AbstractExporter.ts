...
export const Exporter = <T extends AbstractExporter>(name: string) => {
  ...
  return <T extends { new (...args: any[]): AbstractExporter }>(
    constructor: T
  ) => {
...
export default abstract class AbstractExporter {
   ...
   * @private
   */
  private static exporters: Map<string, AbstractExporter> = new Map<
    string,
    AbstractExporter
  >();
  ...
  public static forName(name: string): AbstractExporter {
    if (AbstractExporter.exporters.has(name)) {
      let knownExporter = AbstractExporter.exporters.get(name);
      if (knownExporter) return knownExporter;
    }
   ...
   * @param iob whether corpus should be exported with IOB2 Tag Scheme or not
   * @return a ZIP-Buffer containing the export-result.
   */
  public async export(corpusId: number, iob?: boolean): Promise<Buffer> {
    let tmpDir = tmp.dirSync();
    console.log("Creating temporary directory: ", tmpDir.name);
    try {
      await this.exportWithinTransaction(corpusId, tmpDir.name, iob); // export to tmp dir
      let zip = new AdmZip();
  ...
  @BeginTransaction
  private async exportWithinTransaction(
    corpusId: number,
    targetFolder: string,
    iob?: boolean
  ): Promise<void> {
    return this.doExport(corpusId, targetFolder, iob);
  }
  ...
  public abstract supportsIOB(): boolean;
}
